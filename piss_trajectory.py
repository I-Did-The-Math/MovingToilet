...
ser = serial.Serial('COM3', 2000000)  # Replace 'COM3' with your actual COM port
velocity = 0.0
    ...
    while True:
        ...
        if mode == 'detect_origin':
            ...
            if z > 0:  # Avoid division by zero and invalid depth values
                ...
                cv2.putText(color_image, velocity_text, (10, 180), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)

                # Draw the bottom point on the frame
                cv2.circle(color_image, bottom_centroid, 5, (0, 255, 0), -1)

        # Read the frame from the virtual camera
        ret, virtual_frame = cap.read()
        if ret:
            # Display the virtual camera feed in another window
            cv2.imshow('Virtual Camera Feed', virtual_frame)
